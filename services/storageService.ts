...
import { Client, Invoice, Booking, Expense } from "../types";
...
export const saveToStorage = (clients: Client[], invoices: Invoice[], bookings: Booking[], expenses: Expense[]) => {
  try {
    const data: AppData = {
      clients,
      invoices,
...
export const exportToDrive = (clients: Client[], invoices: Invoice[], bookings: Booking[], expenses: Expense[], email: string) => {
  if (!email) {
  ...
  a.download = `apco_secure_backup_${new Date().toISOString().split('T')[0]}.apco`;
...
export const importFromDrive = async (file: File, email: string): Promise<AppData | null> => {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (data.owner && data.owner !== email) {
            reject("Security Alert: This file does not belong to the registered email ID.");
            return;
          }
